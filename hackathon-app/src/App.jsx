...
function App() {
	...
	useEffect(() => {
		fetch('/data/wellbeing_master.json')
			.then((response) => {
	...
	const avgLifeSatisfaction = (
		data.reduce((sum, item) => sum + item.life_satisfaction, 0) / data.length
	).toFixed(1);
	const avgHappiness = (
		data.reduce((sum, item) => sum + item.happiness, 0) / data.length
	).toFixed(1);
	const avgLoneliness = Math.round(
		data.reduce((sum, item) => sum + item.lonely_at_least_occasionally_pct, 0) /
			data.length,
	);
	const regions = [...new Set(data.map((item) => item.geography))].length;
				...
				<div className='mt-8 bg-white/80 backdrop-blur rounded-lg shadow p-6'>
					...
					<div className='grid grid-cols-1 md:grid-cols-2 gap-6 text-sm text-gray-600'>
						<div>
							...
							<p>
								Personal wellbeing scores are measured on a scale of 0-10,
								covering life satisfaction, worthwhile activities, happiness,
								and anxiety levels across 9 UK regions.
							</p>
						</div>
						<div>
							<h3 className='font-semibold text-gray-700 mb-2'>
							...
							<p>
								Loneliness data shows the percentage of respondents experiencing
								different frequencies of loneliness, from "never" to "often or
								always".
							</p>
						</div>
					</div>
