...
const WellbeingChart = ({ data }) => {
	// Latest quarter: Q4 (int), regions only
	const latestData = data
		.filter((item) => item.quarter === 4 && item.geography_level === 'region')
		.sort((a, b) => a.geography.localeCompare(b.geography));

	const chartData = {
		labels: latestData.map((item) =>
			item.geography.replace(' and The ', ' & '),
		),
		datasets: [
			{
			...
			},
			{
				...
				data: latestData.map((item) => 10 - item.anxiety),
	...
	// Optional: handle empty data (prevents chart errors)
	if (!latestData.length) {
		return (
			<div className='bg-white p-6 rounded-lg shadow-lg'>
				<p className='text-gray-600'>No regional data available for Q4.</p>
			</div>
		);
	}
